/**
 * @file test_6axis_synthetic.c
 * @brief End-to-end validation using synthetic IMU datasets
 *
 * Tests 6-axis sensor fusion algorithm with synthetic IMU data with ground truth
 *
...
double quaternion_distance(const double q1[4], const double q2[4]) {
    ...
    if (dot > 1.0) dot = 1.0;  // Clamp for numerical stability
    return 1.0 - dot;
}
...
void compute_errors(const synthetic_sample_t *sample, const sf_algo_output_t *output,
    ...
    // Confirmed from algo_sf_6x_sensor_fusion.c:262-264:
...
int parse_synthetic_csv(char *line, synthetic_sample_t *sample) {
    // Format: timestamp_ns,accel_x,accel_y,accel_z,gyro_x,gyro_y,gyro_z,
    //         mag_x,mag_y,mag_z,gt_roll,gt_pitch,gt_yaw,gt_qw,gt_qx,gt_qy,gt_qz
...
void print_results(const char *filename, const statistics_t *stats) {
    printf("\n");
    printf("=======================================================================\n");
    printf("6-AXIS E2E TEST RESULTS\n");
    printf("=======================================================================\n");
    printf("Dataset: %s\n", filename);
    ...
    printf("  Roll:  %.3f°\n", stats->roll_rmse);
    printf("  Pitch: %.3f°\n", stats->pitch_rmse);
    printf("  Yaw:   %.3f° (expected drift for 6-axis)\n", stats->yaw_rmse);
    printf("\n");
...
int main(int argc, char *argv[]) {
    if (argc < 2) {
        ...
        printf("  %s test/datasets/synthetic/rotation_z_30dps_10s.csv\n", argv[0]);
        return 1;
    }
    ...
    printf("=======================================================================\n");
    printf("6-AXIS SENSOR FUSION - END-TO-END TEST\n");
    printf("=======================================================================\n");
    printf("Dataset: %s\n", filename);
    printf("Testing: Accelerometer + Gyroscope fusion\n\n");
    ...
    return (stats.roll_rmse < 10.0 && stats.pitch_rmse < 10.0 &&
            stats.angular_error_mean < 15.0) ? 0 : 1;
}
