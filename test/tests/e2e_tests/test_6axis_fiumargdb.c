/**
 * @file test_6axis_fiumargdb.c
 * @brief End-to-end validation using FIUMARGDB real-world dataset
 *
 * Tests 6-axis sensor fusion algorithm with actual IMU data and ground truth quaternions
...
int main(int argc, char *argv[]) {
    ...
    printf("=======================================================\n");
    printf("6-AXIS END-TO-END VALIDATION WITH FIUMARGDB DATASET\n");
    printf("=======================================================\n");
    printf("Dataset: %s\n\n", filename);
    ...
    while (fgets(line, sizeof(line), fp) && sample_count < 100) {
        ...
        // Calculate errors (6-axis only provides roll and pitch)
        ...
        double total_error = sqrt(error_roll * error_roll + error_pitch * error_pitch);
        sum_error += total_error;
        if (total_error > max_error) max_error = total_error;

        int pass = (error_roll < TOLERANCE_DEGREES && error_pitch < TOLERANCE_DEGREES);

        if (sample_count < 5 || !pass) {  // Show first 5 and all failures
            printf("Sample %d (t=%.2fms):\n", sample_count + 1, sample.timestamp);
            printf("  Ground Truth: Roll=%7.2f°  Pitch=%7.2f°\n", gt_roll, gt_pitch);
            printf("  Calculated:   Roll=%7.2f°  Pitch=%7.2f°\n", calc_roll, calc_pitch);
            printf("  Error:        Roll=%7.2f°  Pitch=%7.2f°  Total=%.2f°  %s\n\n",
                   error_roll, error_pitch, total_error, pass ? "✓ PASS" : "✗ FAIL");
        }
