...
def main():
    ...
    sample_count = 0
    target_ts = 4741909075  # Timestamp from C output for sample 654
    ...
    for i, sample in enumerate(sensor_data):
        ...
        # Check if this timestamp matches and measurement_update will run
        will_run_meas = sf.meas_updt_ts < sf.acc_data.timestamp

        if timestamp == target_ts and will_run_meas and sensor_id == 0:
            print(f"\n{'='*80}")
        ...
        if timestamp > target_ts:
            print(f"Passed target timestamp without finding meas_update")
            break
